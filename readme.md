...
1. `\b`：匹配一个零宽单词边界，如一个字母与一个空格之间；例如，`/\bno/` 匹配 `"at noon"` 中的 `"no"`，`/ly\b/` 匹配 `"possibly yesterday."` 中的 `"ly"`

1. `\B`：匹配一个零宽非单词边界，如两个字母之间或两个空格之间；例如，`/\Bon/` 匹配 `"at noon"` 中的 `"on"`，`/ye\B/` 匹配 `"possibly yesterday."` 中的 `"ye"`
...
1. 第2位为`3、4、5、7、8`中的一个：`/[34578]/` 或 `/(3|4|5|7|8)/`

1. 剩余3-11位均为数字，并以数字结尾：`/\d{9}$/`
...
1. domain：任意英文字母（a-z/A-Z）、数字（0-9）、连字符（-），长度大于0

1. extension：任意英文字母（a-z/A-Z），长度2-8

1. optional-extension：`"."`开头，后面跟任意英文字母（a-z/A-Z），长度2-8，可选
...
1. [MDN - Regular Expression](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions)

1. [Microsoft - Regular Expression Reference](https://docs.microsoft.com/en-us/dotnet/standard/base-types/regular-expression-language-quick-reference) 

1. [W3schools - Regexp](https://www.w3schools.com/jsref/jsref_obj_regexp.asp)
